...
import { EntityInspector, selectedEntitiesOf } from "../bridge/ParametricModel";
...
export function PropertiesPalette() {
  ...
  // Entities generated by a component: say what each is, how it was made and
  // what moves it — its coordinates are expressions of values, not numbers to type.
  const selectedIdSet = new Set(state.selectedIds);
  const componentPicks = new Map<string, string[]>();
  for (const inst of state.cad.components) {
    const entities = selectedEntitiesOf(state, inst.id);
    if (entities.length) componentPicks.set(inst.id, entities);
  }
  if (componentPicks.size > 0 && [...selectedIdSet].every((id) => state.shapes.find((sh) => sh.id === id)?.componentInstanceId || state.cad.annotations.find((a) => a.id === id)?.componentInstanceId)) {
    return (
      <div className="flex-1 overflow-y-auto p-3 text-[11px] select-none font-sans flex flex-col gap-3">
        {[...componentPicks.entries()].map(([instanceId, entities]) => (
          <section key={instanceId} className="flex flex-col gap-2">
            <div className="pb-2 border-b border-(--rule) flex items-center justify-between gap-2">
              <span className="font-semibold text-(--fg-primary) truncate">{state.cad.components.find((c) => c.id === instanceId)?.name}</span>
              <span className="text-[10px] text-(--pen) font-mono">{entities.length} selected</span>
            </div>
            <EntityInspector instanceId={instanceId} entities={entities} />
          </section>
        ))}
      </div>
    );
