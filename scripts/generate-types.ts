...
async function generate() {
  ...
  const outputTsDir = path.join(rootDir, "lib", "parametric");
  const outputPyDir = path.join(rootDir, "services", "models");

  fs.mkdirSync(outputTsDir, { recursive: true });
  fs.mkdirSync(outputPyDir, { recursive: true });

  const sketchSchemaPath = path.join(schemasDir, "parametric-sketch.schema.json");
  ...
  const sketchTs = await compileFromFile(sketchSchemaPath, {
  ...
  const templateTs = await compileFromFile(templateSchemaPath, {
    bannerComment: "/* tslint:disable */\n/**\n * This file was automatically generated from template.schema.json.\n * DO NOT MODIFY IT BY HAND. Instead, modify the source JSON Schema file,\n * and run `bun run generate:types` to regenerate this file.\n */\n",
  });

  const combinedTs = `${sketchTs}\n\n${templateTs}`;
  const tsOutputPath = path.join(outputTsDir, "schemaTypes.ts");
  fs.writeFileSync(tsOutputPath, combinedTs, "utf-8");
  console.log(`Successfully generated TypeScript types at ${tsOutputPath}`);

  // Generate Python Pydantic models representation
  const pyOutputPath = path.join(outputPyDir, "schema_models.py");
  const pyCode = `"""
...
`;
  fs.writeFileSync(pyOutputPath, pyCode, "utf-8");
  console.log(`Successfully generated Python models at ${pyOutputPath}`);
}
