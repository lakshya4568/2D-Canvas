...
export interface DrafterState {
  ...
  cad?: CadDocState;
  cadShapes?: Shape[];
  /** The agent's plan and its own construction (see construction.ts). */
...
export class DraftingWorkspace {
  ...
  cadShapes: Shape[];
  /** The agent's plan and the drawing it constructs from it. */
  construction: ConstructionState;
  ...
  private constructionStyle(extra: Partial<LineShape>, flag?: boolean): Partial<LineShape> {
    return flag
  ...
  drawLine(name: string | undefined, a: Pt, b: Pt, construction = false): { id: string; commit: CommitResult } {
    ...
    const shape = this.lineShape(id, name ?? id, a, b, this.constructionStyle({}, construction));
  ...
  drawPolyline(
    ...
    for (let i = 0; i < edges; i++) {
      ...
      made.push(
        this.lineShape(`${group}_${i + 1}`, `${name ?? group} ${i + 1}`, a, b, this.constructionStyle({ groupId: group, groupName: name ?? group }, construction))
      );
    }
