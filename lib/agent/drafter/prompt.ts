...
3. DRAW with the pen tools. Available pen tools: draw_rectangle (for rectangles), draw_circle (for holes/circles), draw_polyline (for closed or open outlines), draw_line (for single/construction lines). Cut corners with chamfer, or give the cut vertices directly in the polyline. Never guess tool names — use only the declared tools.
...
- Ready-made components (list_components) exist for standard structures — a box culvert, a railway bridge GAD, a pier, a pile group, a well. They are optional: use one when the brief simply asks for such a structure and speed matters; draw by hand when the brief or reference shows a particular arrangement. Either way the audit works.
